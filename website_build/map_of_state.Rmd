...
new_states = subset(states, is.element(states$STUSPS, pull(US, state)))
#is.element(new_states$STUSPS,US$state)
#is.element(US$state,new_states$STUSPS)

## make it into same order
US = US[order(match(pull(US, state), new_states$STUSPS)),]
...
pal <- colorNumeric(palette = "Reds",domain = pull(US, Num.UFO))
labels = paste( "<p>","Number of UFO Observed Across Time ：", pull(US, Num.UFO) ,"</p>",
  sep = "")
  leaflet() %>%
  setView(-96,37.8,4) %>%
  addTiles() %>%
  addPolygons(data = states,
              ...
              color = "white",
              fillOpacity = 0.75,
              fillColor = pal(pull(US, Num.UFO)),
              label = lapply(labels, HTML)
              ) %>%
    addLegend(pal = pal,
              values = pull(US, Num.UFO),
              opacity = 0.7,
              position = "topright")
```
